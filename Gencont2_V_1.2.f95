...
Implicit none
	...
	integer			:: ninp, inp(30), k_cand, ncand_, iout(30), nout
	integer			:: row
	real(8)			:: gen_gain, PaveA 
    	real(8)			:: PaveA_selct
	...
	real(8), dimension (:), allocatable 		:: aveA, c, ebv, c_young,w_young,covA
...
! Control file variables
  	character(len=100) 			:: ped_file, data_file , Parafile,  outfile, a_file_name, A_prev
  	integer 				:: nsex, ncand, nped, niter, nprog, minimize, nanim,rerun
	...
	real(8), dimension (30)			:: cmax_group, cmin_group, s_group  
...
!print*, str_out(i)(1:7)
       		if(str_out(i)(1:4)=='name' .or. str_out(i)(1:4)=='NAME')then
           			...
           			inp(i-2)=5
       		else if( str_out(i)(1:5)=='c_max' .or. str_out(i)(1:4)=='C_MAX')then
           			inp(i-2)=6
				restrict(1)= 1
       		else if( str_out(i)(1:)=='c_min' .or. str_out(i)(1:4)=='C_MIN')then
           			inp(i-2)=7
				restrict(2) = 1
...
allocate (data_info(ncand_),covA(nsex))
...
!print*,'l =', inp(1:20)
!print*, 'sum of cmax', data_info(1:20)%c_max
!stop
...
if(rerun == 1) then
	CALL readin_half (A_anim, nanim, A_prev)
else
	if (vanraden == 1) then 
		...
		if(data_info(ii)%avail == 1 .OR. data_info(ii)%c_prev > 0) then
			if(data_info(ii)%sex==kk) then
				...
				do i=1, nout
					...
					if(iout(i)==4) then
						write(line(j:j+10), '(f10.3)') abs(data_info(ii)%c_min*100.)
						j=j+10
					end if
					if(iout(i)==5) then
...
! Define related parameters 
	...
	integer								:: i, j, x, xx,xxx, n_run, nin, nout, y,niter_1, md,b 
	integer								:: select_all_group(20), ii, jj, ibig, iter_c, nsex_1
	real(8)								:: k1,threshold, big, zero, c2a22
	integer, dimension (ncand)	 				:: locate,loc_post, rem_post
	integer, dimension(nsex)					:: nrem, nin_check, dd
	!integer, dimension(:), allocatable				:: nrem, nin_check, dd
   	integer, dimension (:), allocatable				:: in, out, q_loc 
	integer, dimension (ncand,nsex)					:: cmin_check, d_check
...
! Allocate respective dimensions for the array
    	...
    	allocate (ones_r(nsex), q_loc(nsex))
	...
	a_c=0.; k1=0.; c2a22=0.
	select_all_group=0; nin_check=0.
	!q_loc(1:nsex)=(/(i,i=1,nsex)/)
	niter_1=niter; cmin_check=0; md=0; b=0
...
!print*, cand_group
!stop
!print*, 'select_all_group', select_all_group(1:nsex)
!stop
!print*,'nsex =', nsex
...
!print*, 'sum(ones_r) ', ones_r
!print*, 'sum(c) 1', sum(c)

	!allocate(nrem(nsex_1), nin_check(nsex_1), dd(nsex_1))
	nrem=0; nin_check=0; dd=0

!print*, select_all_group
!print*, 'nsex_1', nsex_1
!print*,'size of QD', size(QD,1), size(QD,2)
!print*,'q_loc', q_loc(1:nsex_1)
!print '(30F6.3)', Q
!stop
!print*, 'ones_r =', ones
!stop
...
!stop
!print*
!print '(30F6.3)', cmax(locate(1:xx))
!stop
!print '(17F6.3)', cmin(locate(1:xx))

  	CALL oc_iter (locate, c,c_set, k1, ebv,  a_c, QD, A, niter_1, n_run, ones_r, ncand, nsex_1, &
			xxx, Error_c, thold, round, restrict, islow)

	
!*** If there is no restriction on cmax and cmin, Exit ***
	...
	nin=xx
!exit
!print*, 'sum(c_store)', sum(c_store)
!*** if enough animals are selected based on user setup then exit

	do i=1,nsex_1
		if(iter_c .eq. 1) then
			nrem(i)= abs(ngroup(i) - (cand_group(i) - nin_check(i)))
		else
			nrem(i)= ngroup(i) - (cand_group(i) - nin_check(i))
		end if

		!if (all(nrem .EQ. 0)) then
		!	print*, 'goodbye'
		!	exit
		!end if
	end do
...
!*** If there is restriction, deal with cmax first ***
	...
	IF(restrict(1) == 1 .or. restrict(2) /= 0) THEN
		...
		nin_check=0
  		threshold=0.
  		if(islow==1)threshold=max(maxval(c_set(locate(1:xx)) - cmax(locate(1:xx))),0.)
...
!print*, 'ii =',ii
		...
		! Adjust ones:
      				ones_r= ones_r - QD(ii,:)*cmax(ii)
    			else
      				nin=nin+1
      				in(nin)=i
				b=maxloc(QD(ii,:),1)
				nin_check(b)=nin_check(b)+1
    			end if
  		end do
!print*, 'Here 2'
!print*, 'the xx is ', xx
!print*, 'the nin is ', nin
		...
		if(nin==xx .and. restrict(2)/=0) then
			...
			if(any(cmin < 0))then  !fix their solutions to abs(cmin(i))
				...
				do i=1,xx
					if(cmin(locate(i)) .LT. 0.) then
						b=maxloc(QD(locate(i),:),1)
						dd(b)=dd(b)+1
						cmin_check(dd(b),b)=locate(i)
						d_check(dd(b),b) = i
					end if
				end do
!print*, 'dd =', dd
!print'(17i6)', locate(1:xx)
!print '(17i6)',d_check(1:dd)
!print '(17i6)', cmin_check(1:dd)
!print'(1780F6.3)', c_set(cmin_check(1:dd))
!dd=maxloc(c_set(cmin_check(1:dd)),1)
!print*, 'dd =', dd
!print*, cmin_check(dd)
!print*,c_set(cmin_check(dd))
!stop
!print*, 'nrem =', nrem  
!print*, 'nsex_1',nsex_1   

		!if (all(nrem .EQ. 0) .OR. all(dd .EQ. 0)) then
		!	!print*, 'goodbye'
		!	exit
		!end if
 			
      			do j=1, nsex_1
				if (nrem(j) .GT.0 .and. dd(j) .GT.0) then
      				do i=1,nrem(j)
					md=maxloc(c_set(cmin_check(1:dd(j),j)),1)
					ii=cmin_check(md,j)
!print*, 'ii =',ii			
!print*, cmin_check(ii)
!print*, d_check(ii)
			        	!if(cmin(ii) < 0 .and. c_set(ii) > 0.) then    !fix solution to -cmin
							!print*, cmin(ii)
							!print*, ii
			        		c_store(ii)=-cmin(ii)
				...
				! Adjust ones_r :
			        		ones_r = ones_r - QD(ii,:)*c_store(ii)

			        		nin=nin - 1
						in(d_check(md,j):nin)=in(d_check(md,j)+1:nin+1)
			            		dd(j) = dd(j)-1
			        		cmin_check(md:dd(j),j) = cmin_check(md+1:dd(j)+1,j)
			          	!end if                  
      				end do
				end if
			end do
...
!print*, 'the nin is ', nin
	...
	xx=nin
!print*, 'ones_r =', ones_r(1:nsex_1)
if(all(ones_r(1:nsex_1) <= 1E-5)) exit
