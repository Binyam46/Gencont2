...
Subroutine oc_iter (locate, c,c_set, K2, ebv, a_c, Q, A, niter, n_run, ones_1, ncand, nsex, & 
	...
	real(8), dimension (:,:), allocatable 				:: Q_n, tQ, QQ, QQII, QQII_1
      	...
      	allocate (ACCA(xxx), RR(nsex), deltC(xxx), deltlamda(nsex))
...
!stop
	...
	allocate (Q_n(xxx, nsex_1), tQ(nsex_1, xxx), QQ(nsex_1, nsex_1), QQII(nsex_1, nsex_1))
	allocate (delr(nsex_1),lamda(nsex_1), r_0(nsex_1), sse(nsex_1), QQII_1(nsex,nsex_1), sse_2(3))
...
! Check and correct the ones and Q
	...
	do i=1,nsex
		...
		if(rem_animals(i) > 0) then
			j=j+1
			Q_n(:,j)=Q(locate_in(1:xxx), i)
			r_0(j) = ones_1(i)
		end if
	end do
...
!if(mod(round,100)==0) print*,'round', round
	!temp_c=c_set 
    	...
    	if (round .GT. n_run+1) then
  	!	if(islow==1) threshold=maxval()
			x=size(c(1:xxx),1)
  			xxx=0
			n_run = n_run +100
   			do j= 1, x
...
!print*, 'lamda =', lamda
	...
	if (mini== 1) then
		ACCA = matmul(Q_n,lamda)/(2*lamda0) + aac(1:xxx)
	else
		ACCA = ((ebv_n(1:xxx) - matmul(Q_n,lamda))/(2*lamda0))- aac(1:xxx)
...
!write(32,*) sse_2(1), sse_2(2), sse_2(3)
!222 format(3f12.10)  
...
if(mini==1) then
        ...
        if ((sse_2(1) .LT. thold) .and. (sse_2(2) .LT. 1E-3) .and. (sse_2(3) .LT. 1E-3) ) then
      
          	Error_c=0
          	!print*, 'Converged after', round, ' itrations'
          	exit 
           
        else if (niter == round .and. sse_2(1) .GE. thold) then
          	Error_c=1
